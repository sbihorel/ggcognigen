...
To display the data distribution as density or percentages, we suggest that you use the `after_stat` function to define the `y` aesthetic in your `geom_histogram` call. For `geom_histcount()`, you must also set the `label` aesthetic to `after_stat(density_label)` or `after_stat(percent_label)`.
...
ggpubr::ggarrange(
  ...
  ggplot(data = histdata) +
    aes(x = RANDOM, fill = GROUP) +
    geom_histogram(aes(y = after_stat(100 * count / sum(count))), bins = 15) +
